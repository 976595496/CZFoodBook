...
- (instancetype)initWithSteps:(NSArray *)steps WithInterger:(NSInteger)inter{
    if (self = [super init]) {
        ...
        [weakSelf.showRecipeScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(weakSelf.mas_top).with.offset(80);
        ...
        self.showRecipeScrollView.contentSize = CGSizeMake(self.showRecipeScrollView.frame.size.width, (self.scrollRect.size.height-200)*steps.count);
        ...
        [self.upBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(weakSelf.mas_top).with.offset(20);
        ...
        [self.downBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(40, 40));
            make.bottom.mas_equalTo(weakSelf.mas_bottom).with.offset(-60);
            make.centerX.mas_equalTo(weakSelf.mas_centerX);
        }];
...
- (void)showNumberpage:(NSInteger)numpage{
    ...
    self.showRecipeScrollView.contentOffset = CGPointMake(0, (numpage-1) *self.showRecipeScrollView.frame.size.height);
...
- (void)recipeScrollUp{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.showRecipeScrollView.contentOffset = CGPointMake(0, (self.inter- 2)*self.showRecipeScrollView.frame.size.height);
    }];
...
- (void)recipeScrollDown{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.showRecipeScrollView.contentOffset = CGPointMake(0, self.inter*self.showRecipeScrollView.frame.size.height);
    }];
...
- (void)addShowRecipeStepView{
    for (int i = 0; i<self.steps.count; i++) {
        ...
        [stepView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerX.mas_equalTo(self.showRecipeScrollView.mas_centerX);
            make.centerY.mas_equalTo(self.showRecipeScrollView.mas_centerY).with.offset(i*(self.scrollRect.size.height-200));
        }];
    }
}
